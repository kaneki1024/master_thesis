...
\tableofcontents
  \chapter{序論}
	...
	\chapter{実験手法}
		...
		\begin{figure}[h]
			\centering
			\includegraphics[scale=0.7]{試料作製手順.jpg}
	...
	\chapter{付録}
	\section{DSC}
	本研究ではシルクフィルムの熱特性を評価するために
	示差走査熱量測定(DSC、Differential Scanning Calorimetry)を使用した。
	DSCとは図\ref{DSC概念図}の様に参照試料と測定試料を同じ温度で走査し、その熱量差を測定する。
	参照試料として空のアルミパンを使用した。
	\begin{figure}[h]
		\centering
		\begin{minipage}{0.45\hsize}
			\centering
			\includegraphics[scale=0.9]{DSC.jpg}
			\caption{DSC装置の概念図}
			\label{DSC概念図}
		\end{minipage}
		\begin{minipage}{0.45\hsize}
			\centering
			\includegraphics[scale=0.15]{DSC_hitachi.jpg}
			\caption{DSC7020(Hitachi Corporation)}
		\end{minipage}
	\end{figure}
	\newpage
	\section{FTIR}
