...
\tableofcontents
  \chapter{序論}
		...
		この生糸の状態ではセリシンを多く含み、精錬、合糸、撚糸を行い絹糸が作成される。
		...
		分類学において昆虫は31目に分類され、そのうち約1/3は繭糸や糸を作成する。
		よって野外でも繭を作成する生物は存在し、
		...
		絹糸は外科手術の縫合糸として利用されたという実績が存在し、生体親和性が高いされる。
		よってその親和性を利用して図\ref{silk_device}の(A)人工血管、(B)軟骨再生材料としてのスポンジ、
		(C)創傷保護材としてのフィルムなどに利用されている。
		また、図\ref{silk_device}(D)はパウダー状にしたシルクを熱プレスして作成された焼結体であり
		優れた誘電体になる。電子回路の基盤として利用が検討されている。
		\begin{figure}[H]
			\centering
			\includegraphics[width=\linewidth]{silk_device.jpg}
			\caption{衣服以外のシルクの利用例。(A)人工血管、(B)シルクスポンジ、(C)フィルム、
			(D)電子回路基盤。}
			\label{silk_device}
		\end{figure}
		\newpage
		...
		絹糸を構成する一つの蛋白質であるフィブロインは図\ref{fibroin}(b)の様に分子量の異なる3個の分子により
		構成されている。
		分子量 360 kDaのH鎖と
		...
		\begin{figure}[h]
			...
			\caption{絹糸の構造。(a) 絹糸1本の構造, (b) フィブロインにおけるH鎖とL鎖の関係, 
			(c) H鎖周期構造におけるアミノ酸配列。}
			\label{fibroin}
		\end{figure}

		絹糸におけるセリシンは、図\ref{fibroin}(A)の通りフィブロインを囲む形に存在している。
		...
		絹糸の構造研究はセリシンが結晶構造を取らないため絹糸のフィブロイン構造を指す場合が
		...
		逆平行$\beta$シートが形成されている。絹糸の二次元XRDは図\ref{silk_fiber_debai}の通りであり、デバイシェラー環から
		配向による異方性が伺える\cite{yoshioka2016molecular}。
		\begin{figure}[h]
			\centering
			\includegraphics[scale=0.8]{silk_fiber_デバイ.jpg}
			\caption{絹糸の2次元XRD\cite{yoshioka2016molecular}。左下の数字はHermanの配向係数。}
			\label{silk_fiber_debai}
		\end{figure}
		絹糸の構造はMarshらによって逆平行$\beta$シートのモデルが提案され長く採用されてきた\cite{marshモデル}。
		しかしそのモデルのみだとX線回折像を説明しきれない箇所があるとされている。
		AsakuraらはNMRを用いて構造解析を行い、絹糸は分子間間隔の異なる2種類の
		...
		\newpage
		\subsection{絹糸の圧電性}
			絹糸の圧電性に関して初めて言及したのはHarveyとされている\cite{Harvey}。
	...
	\chapter{原理}
		...
		これらを式にまとめると
			...
			組み紐に組み込み、ファッショナブルな身体計測デバイスを作成し、脈波などの身体情報を入手した
			...
			$\beta$ シート構造も取りうる。本研究の材料である絹糸は逆平行$\beta$シートを構成する。
			...
			\begin{figure}[h]
				\centering
				\includegraphics[width=\linewidth]{anti_parallel_beta.jpg}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=\linewidth]{anit_parallel_force.jpg}
	...
	\chapter{結果と考察}
			...
			\begin{figure}[H]
				...
				\caption{角度別弾性率の違い.(A)式\ref{ずりの圧電共鳴_理論式}に基づいてfittingして得られた角度別の弾性率。
			...
			である。正方形試料の一辺の長さ$l$、密度$\rho$と式\ref{ずりの共鳴周波数}を用いて共鳴周波数から
		...
		微小な減少傾向である。
			\begin{figure}[H]
				...
				(A)電気機械結合係数と温度の関係、(B)圧電e定数と温度の関係、(C)圧電d定数と温度の関係、
				(D)束縛誘電率$\varepsilon'_s$と温度の関係。}
				\label{温度_k_e_d_セリシンなし}
			\end{figure}
			\newpage
			0℃以下の圧電e定数においては弾性率の低下と誘電率の上昇により微小な減少、80℃以降は緩やかな弾性率の変化
			と緩やかな誘電率の変化で同様に緩やかな変化となる。一方で、0℃から60℃付近は誘電率の大きな変動と弾性率の大きな変動により
			圧電e定数は激しい変動となる。圧電d定数は式\ref{e_to_d}に基づいて計算され、対応する値となる。
			...
			である。
			正方形試料の一辺の長さ$l$、密度$\rho$と式\ref{ずりの共鳴周波数}を用いて共鳴周波数から
			...
			複数の共鳴を１つの共鳴とみなして式\ref{ずりの圧電共鳴_理論式}を用いてフィッティングした。
			フィッティングで使用した電気機械結合係数$k_{14}$、フィッティングにより得られる
			圧電e定数$e_{14}$、圧電d定数$d_{14}$、束縛誘電率$\varepsilon'_{S}$
			...
			\begin{figure}[h]
				...
				(A)温度と電気機械結合係数の関係、(B)温度と圧電e定数の関係、(C)圧電d定数と温度の関係、
			...
			\begin{figure}[h]
				...
				(A) 計測した40 Hzから110 MHzにおける周波数分散、(B) 圧電共鳴付近の様子。}
				\label{熱処理後の誘電率}
			\end{figure}
	\chapter{総括}
	\chapter{付録}
	...
	セリシンを除去していないフィルムを作成する場合は図\ref{フィブロインキャストフィルムの作成方法}の右側の
	溶解過程のみ行えばよい。
	\begin{figure}[h]
		\centering
		\includegraphics{cast_film_作成方法.jpg}
		\caption{キャストフィルムの作成方法\cite{フィブロイン溶解方法}.}
		\label{フィブロインキャストフィルムの作成方法}
	\end{figure}
	\newpage
	キャストにて作成した試料は以下の通りである。
	セリシンを除去した場合は透明になると判明した。
	\begin{figure}[h]
		\centering
		\includegraphics{cast_film_sample.jpg}
		\caption{キャストで作成した試料. (A)セリシンを除去しなった試料, (B)セリシンを除去した試料.}
	\end{figure}
	\newpage
	...
	中嶋研究室、2022年修了の横山様にはデータサイエンスという
	...
	中嶋研究室、2022年修了の井桁様にはスライド作成など研究室活動の基本を教えて頂きました。
	感謝を申し上げます。
