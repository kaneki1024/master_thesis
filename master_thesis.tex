...
\tableofcontents
  \chapter{序論}
		...
		熱プレス等にて作成された焼結体であり優れた誘電体になる\cite{焼結,焼結2}。
	...
	\chapter{原理}
		...
		これらを式にまとめると
			...
			圧電$d$形式、式\ref{圧電d形式1}から自由状態($T=0$)において電源から
			\begin{equation}
				D=\varepsilon^T E
			\end{equation}
			...
			電場に対するひずみは圧電$d$形式、式\ref{圧電d形式1}において$T=0$として以下の通りである。
			\begin{equation}
				S = d E
			\end{equation}
			単位体積あたりの機械エネルギーは
			...
			式\ref{圧電d形式1}の圧電$d$形式の2式目を以下のように応力$T$を求める形に式変形する。			
			\begin{equation}
	...
	\chapter{結果と考察}
			...
			\end{equation}
			入手した圧電共鳴スペクトルに対し、式\ref{ずりの圧電共鳴_理論式}に基づいてフィッティングした
			後に上式から弾性率を計算した。
		...
		微小な減少傾向である。
			...
			\end{figure}
			\newpage
			0℃以下の圧電$e$定数においては弾性率の低下と誘電率の上昇により微小な減少、80℃以降は緩やかな弾性率の変化
			と緩やかな誘電率の変化で同様に緩やかな変化となる。一方で、0℃から60℃付近は誘電率の大きな変動と弾性率の大きな変動により
			圧電$e$定数は激しい変動となる。圧電$d$定数は式\ref{e_to_d}に基づいて計算され、対応する値となる。
	...
	中嶋研究室、2022年修士課程修了の井桁様にはスライド作成など研究室活動の基本を教えて頂きました。
	感謝を申し上げます。
